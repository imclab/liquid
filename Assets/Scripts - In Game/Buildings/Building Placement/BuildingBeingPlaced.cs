...
public class BuildingBeingPlaced : MonoBehaviour {
	...
	void Start () {
		...
		if (renderer != null) {
			m_BuildingMaterials.AddRange (renderer.materials);
		}
		...
		if (renderer != null) {
			lowerBound.x = Mathf.Min (lowerBound.x, renderer.bounds.min.x);
			lowerBound.y = Mathf.Min (lowerBound.y, renderer.bounds.min.y);
			lowerBound.z = Mathf.Min (lowerBound.z, renderer.bounds.min.z);
			
			upperBound.x = Mathf.Max (upperBound.x, renderer.bounds.max.x);
			upperBound.y = Mathf.Max (upperBound.y, renderer.bounds.max.y);
			upperBound.z = Mathf.Max (upperBound.z, renderer.bounds.max.z);
		}

...
public class MainCamera : MonoBehaviour, ICamera {
	...
	public void Zoom(object sender, ScrollWheelEventArgs e) {
		camera.fieldOfView -= e.ScrollValue*ZoomRate*Time.deltaTime;
		
		if (camera.fieldOfView < m_MinFieldOfView) {
			camera.fieldOfView = m_MinFieldOfView;
		}
		else if (camera.fieldOfView > m_MaxFieldOfView) {
			camera.fieldOfView = m_MaxFieldOfView;
		}
